...
@section('content')
<div class="py-12 bg-light" x-data="{ step: 1, email: '', website: '', terms: false, bizName: '', bizType: 'clothing', bizSales: '' }">
        ...
        <form action="/merchant/signup" method="POST">
            ...
            <div x-show="step === 1">
                ...
                <div class="mb-6">
                    ...
                    <input type="text" name="website" x-model="website" class="w-full border rounded-lg p-2.5 focus:outline-none focus:border-primary text-left" dir="ltr" placeholder="digikala.com">
                </div>
                <button type="button" @click="if(email.trim() !== '' && website.trim() !== '') { step = 2 } else { alert('لطفاً ابتدا ایمیل و آدرس وبسایت را وارد کنید.') }" class="w-full bg-primary text-white py-2.5 rounded-lg font-bold hover:bg-opacity-90 transition">مرحله بعدی</button>
            </div>
            ...
            <div x-show="step === 2" x-cloak>
                ...
                <div class="flex space-x-4 space-x-reverse">
                    <button type="button" @click="step = 1" class="w-1/2 bg-gray-100 text-gray-700 py-2.5 rounded-lg font-bold hover:bg-gray-200 transition">قبلی</button>
                    <button type="button" @click="if(terms) { step = 3 } else { alert('لطفاً ابتدا تیک قبول قوانین را بزنید.') }" class="w-1/2 bg-primary text-white py-2.5 rounded-lg font-bold hover:bg-opacity-90 transition">مرحله بعدی</button>
                </div>
            </div>
            ...
            <div x-show="step === 3" x-cloak>
                <div class="mb-4">
                    ...
                    <input type="text" name="biz_name" x-model="bizName" class="w-full border rounded-lg p-2.5 focus:outline-none focus:border-primary" placeholder="مثلا: پوشاک تن‌پوش">
                </div>
                <div class="mb-4">
                    <label class="block text-gray-700 text-sm font-bold mb-2">حوزه فعالیت:</label>
                    <select name="biz_type" x-model="bizType" class="w-full border rounded-lg p-2.5 focus:outline-none focus:border-primary">
                ...
                <div class="mb-6">
                    ...
                    <select name="biz_sales" x-model="bizSales" class="w-full border rounded-lg p-2.5 focus:outline-none focus:border-primary">
                ...
                <div class="flex space-x-4 space-x-reverse">
                    <button type="button" @click="step = 2" class="w-1/2 bg-gray-100 text-gray-700 py-2.5 rounded-lg font-bold hover:bg-gray-200 transition">قبلی</button>
                    <button type="button" @click="if(bizName.trim() !== '' && bizType !== '' && bizSales !== '') { step = 4 } else { alert('لطفاً تمامی مشخصات فروشگاه را تکمیل کنید.') }" class="w-1/2 bg-primary text-white py-2.5 rounded-lg font-bold hover:bg-opacity-90 transition">مرحله بعدی</button>
                </div>
            </div>
            ...
            <div x-show="step === 4" x-cloak>
                <div class="mb-4">
                    ...
                    <input type="text" name="name" required class="w-full border rounded-lg p-2.5 focus:outline-none focus:border-primary">
                </div>
                <div class="mb-4">
                    <label class="block text-gray-700 text-sm font-bold mb-2">شماره موبایل مالک:</label>
                    <input type="text" name="owner_phone" required class="w-full border rounded-lg p-2.5 focus:outline-none focus:border-primary text-left" dir="ltr" placeholder="09151234567">
                </div>
                <div class="mb-6">
                    <label class="block text-gray-700 text-sm font-bold mb-2">کلمه عبور برای لاگین‌های بعدی:</label>
                    <input type="password" name="password" required class="w-full border rounded-lg p-2.5 focus:outline-none focus:border-primary text-left" dir="ltr">
                </div>

                <div class="flex space-x-4 space-x-reverse">
                    <button type="button" @click="step = 3" class="w-1/2 bg-gray-100 text-gray-700 py-2.5 rounded-lg font-bold hover:bg-gray-200 transition">قبلی</button>
                    <button type="submit" class="w-1/2 bg-accent text-white py-2.5 rounded-lg font-bold hover:bg-opacity-90 transition shadow-md">شروع همکاری و دریافت API</button>
                </div>
            </div>
        </form>

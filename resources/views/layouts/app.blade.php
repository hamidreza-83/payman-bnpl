...
<body class="bg-light text-gray-800 flex flex-col min-h-screen">
    <div class="bg-gray-100 border-b border-gray-200 py-1.5 text-xs font-medium text-gray-600">
        <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 flex justify-start space-x-4 space-x-reverse">
            <a href="{{ url('/') }}" class="pb-0.5 transition-colors {{ !request()->routeIs('business.landing') ? 'text-primary border-b-2 border-primary font-bold' : 'hover:text-primary' }}">
                برای خریداران (Shoppers)
            </a>
            
            <a href="{{ route('business.landing') }}" class="pb-0.5 transition-colors {{ request()->routeIs('business.landing') ? 'text-primary border-b-2 border-primary font-bold' : 'hover:text-primary' }}">
        ...
        <div>
            @auth
                <div class="flex items-center space-x-4 space-x-reverse">
                    <a href="/merchant/dashboard" class="bg-accent text-white font-bold py-2 px-6 rounded-full transition-all text-sm">
            ...
            @else
                <a href="/merchant/login" class="border-2 border-accent text-accent hover:bg-accent hover:text-white font-bold py-2 px-6 rounded-full transition-all duration-300">
                    ورود
                </a>
            @endauth
        </div>  
